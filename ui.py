...
def get_shift_predictor():
    weight_file_name = 'trained_weights/shift_in_{}_{}_{}_{:1.3f}.pt'.format('w' if in_W else 'z', training_name,lbl_ids,faithfulness_ratio)
...
def main():
    ...
    options = ["Bald", "Male", "Smiling", "Wearing Hat", "Young"]
    counters = ["Hairy", "Female", "Not Smiling", "Not Wearing Hat", "Old"]
    available_options = []
    # Main content
    ...
    if st.button("Generate Image"):
        ...
        y_target[index1] = 1 - y_target[index1]
        y_target[index2] = 1 - y_target[index2]

        y_target = y_target.int()
        st.subheader("Generated Image")
        st.image(generated_image, use_column_width=True)
        # y_target = [1, 0, 1, 0, 0]
        # for i, tar in enumerate(y_target):
        #     if tar:
        #         # st.sidebar.checkbox(counters[i])
        #         available_options.append(counters[i])
        #     else:
        #         # st.sidebar.checkbox(options[i])
        #         available_options.append(options[i])
        ...
        #     selected_options = st.form_submit_button(label="Submit")
    ...
    with col2:
        ...
        # counterfactual_image = np.ones((256,256,3))
        st.image(counterfactual_image, use_column_width=True)
